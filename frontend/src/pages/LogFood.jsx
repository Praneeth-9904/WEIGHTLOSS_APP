...
export default function LogFood() {
  ...
  const fetchLogs = async () => {
    ...
    if (!token) return;
    try {
      const res = await fetch("https://weightlossapp-production.up.railway.app/my_logs", {
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const res = await fetch("https://weightlossapp-production.up.railway.app/log_food", {
        method: "POST",
        headers: { 
